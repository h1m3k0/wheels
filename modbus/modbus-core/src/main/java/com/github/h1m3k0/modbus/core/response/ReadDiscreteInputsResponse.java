package com.github.h1m3k0.modbus.core.response;

import com.github.h1m3k0.modbus.core.errorres.ReadDiscreteInputsError;
import com.github.h1m3k0.modbus.core.function.ReadDiscreteInputsFunction;
import com.github.h1m3k0.modbus.core.request.ReadDiscreteInputsRequest;
import com.github.h1m3k0.modbus.core.utils.ModbusUtil;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.experimental.Accessors;

@EqualsAndHashCode(callSuper = true)
@Data
@Accessors(chain = true, fluent = true)
public class ReadDiscreteInputsResponse extends ModbusResponse<ReadDiscreteInputsFunction, ReadDiscreteInputsRequest, ReadDiscreteInputsResponse, ReadDiscreteInputsError> {
    private final int quantity;  // 临时
    private final byte[] bytes;  // 封装数据
    private final boolean[] bits;  // 解析数据

    private ReadDiscreteInputsResponse(int quantity, byte[] bytes, boolean[] bits) {
        super.function = ReadDiscreteInputsFunction.Instance();
        this.quantity = quantity;
        this.bytes = bytes;
        this.bits = bits;
    }

    public ReadDiscreteInputsResponse(int quantity, byte[] bytes) {
        this(quantity, bytes, ModbusUtil.bytesToBits(bytes, quantity));
    }

    public ReadDiscreteInputsResponse(byte[] bytes) {
        this(bytes.length << 3, bytes);
    }

    public ReadDiscreteInputsResponse(boolean[] bits) {
        this(bits.length, ModbusUtil.bitsToBytes(bits), bits);
    }
}
